...
begin
  ...
  -- which dumps the proposed row). This also keeps the CHECK a pure backstop, and keeps
  -- the server in exact agreement with the client-side parsePhone, which applies the same
  -- NANP rules -- do not let this drift into a digit-count-only check again.
