...
**Files:**
- Create: `eval/invariance/report.ts`, `scripts/run-invariance.mjs`
...
```js
// scripts/run-invariance.mjs
// Writes the aggregate invariance report. Run: npm run eval:invariance
...
const { runAllAxes } = await import('../eval/invariance/axes.ts');
const { renderInvarianceMarkdown, renderInvarianceJson } = await import('../eval/invariance/report.ts');

const stamp = new Date().toISOString();
const results = runAllAxes();

mkdirSync('eval/reports', { recursive: true });
writeFileSync('eval/reports/invariance.md', renderInvarianceMarkdown(results, stamp));
writeFileSync('eval/reports/invariance.json', renderInvarianceJson(results, stamp));

console.log(renderInvarianceMarkdown(results, stamp));
process.exit(results.every((r) => r.pass) ? 0 : 1);
```

**If `ts-node` is not installed** (check with `node -e "require.resolve('ts-node')"`), do **not** add it — Global Constraints forbid dependency changes. Instead replace `scripts/run-invariance.mjs` with a Jest-driven reporter: add `eval/invariance/__tests__/report-writer.test.ts` that calls `runAllAxes()`, writes the two files via `fs`, and asserts they are non-empty. Then set the npm script to `jest eval/invariance/__tests__/report-writer.test.ts`.

Add to `package.json` `scripts`:
```json
"eval:invariance": "node scripts/run-invariance.mjs"
```
...
```bash
git add eval/invariance/report.ts eval/invariance/__tests__/report.test.ts \
        scripts/run-invariance.mjs package.json eval/reports/invariance.md eval/reports/invariance.json
