// src/features/recommend/makeup/makeup-chat-prompt.ts
// Builds the constrained system prompt for the makeup / shade Q&A domain (CLAUDE.md §0/§1/§3).
//
// Compliance boundary: the context is the user's ALREADY-DERIVED shade descriptors + their
// structured Setup preferences — WORDS only. The raw on-device read, the image, and the numeric
// shade depth NEVER cross into this prompt (shade depth is rendered as a word, its shadeName).
//
// Deterministic-ranking rule: the model is explicitly told NOT to rank, score, or recommend
// specific products. Product ranking is owned by the pure match engine (features/match/*); the
// chat only explains shade / undertone / finish / coverage in cosmetic terms.
import {
  GOAL_LABELS,
  COVERAGE_LABELS,
  SKIP_LABELS,
...
export type { ChatTurn } from '../chat/prompt';
